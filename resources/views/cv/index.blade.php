...
@section('content')
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-9 desctop-992-pr-16">
                        ...
                        <div class="vakancy-page-block company-list-search__block resume-list__block p-rel mb16">
                            <div class="company-list-search__block-left">
                                <div class="resume-list__block-img mb8">
                                    <img src="{{ asset($cv->account->photo) }}" alt="profile">
                                </div>
                            </div>
                            <div class="company-list-search__block-right">
                                <div class="mini-paragraph cadet-blue mobile-mb12">Обновлено 1 апреля 2020 в
                                    15:00 {{ $cv->updated_at }}
                                </div>
                                <h3 class="mini-title mobile-off">{{ $cv->specialization->specialization }}</h3>
                                <div class="d-flex align-items-center flex-wrap mb8 ">
                                    ...
                                    <span class="mr16 paragraph">{{ $cv->account->age }}
                                        {{trans_choice('messages.years', $cv->account->age<21 ? $cv->account->age: substr($cv->account->age, -1)) }}
                                        </span>
                                    <span class="mr16 paragraph">{{ $cv->account->city->city_name }}</span>
                                </div>
                                <p class="paragraph tbold mobile-off">Последнее место работы</p>
                            </div>
                            <div class="company-list-search__block-middle">
                                @if($cv->experiences->first())
                                    <h3 class="mini-title desktop-off">PHP разработчик</h3>
                                    <p class="paragraph mb16 mobile-mb32">
                                        {{ $cv->experiences->first()->position }}
                                        в {{ $cv->experiences->first()->organization }},
                                        с {{ $cv->experiences->first()->start_date }}
                                        по {{ $cv->experiences->first()->stop_date }}
                                        срок {{ $cv->experiences->first()->range }}
                                    </p>
                                @else
                                    <p class="paragraph mb16 mobile-mb32">
                                        Нет опыта работы
                ...
                <ul class="dor-pagination mb128">
                    <li class="page-link-prev"><a href="#"><img class="mr8"
                                                                src="images/mini-left-arrow.svg" alt="arrow">
                            Назад</a>
                    </li>
                    <li><a href="#">1</a></li>
                    ...
                    <li class="page-link-next"><a href="#">Далее <img class="ml8"
                                                                      src="images/mini-right-arrow.svg"
                                                                      alt="arrow"></a>
                    </li>
                </ul>
            </div>
            {{--Filters section--}}
            <div
                class="col-lg-3 desctop-992-pl-16 d-flex flex-column vakancy-page-filter-block vakancy-page-filter-block-dnone">
                <div
